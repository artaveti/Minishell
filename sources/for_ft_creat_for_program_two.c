...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   for_ft_creat_for_program_two.c                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: artaveti <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/07 11:34:51 by artaveti          #+#    #+#             */
/*   Updated: 2024/01/07 11:48:44 by artaveti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lib_for_minishell.h"

void	ft_creat_token_for_redir(t_token_list *redir_list,
			char *string, int type);
void	ft_assign_word_to_argv(t_token_list *token_list,
			char ***argv_for_execve);
void	ft_assign_string_to_word_of_argv(t_token_list *token_list,
			char ***argv_for_execve);
...
t_token_list	*ft_creat_redir_list_for_execve(t_token_list *token_list)
{
	...
	while (token_list != NULL)
	{
		if (token_list->type == REDIR_INT)
			...
			ft_creat_token_for_redir(redir_list, token_list->value,
				REDIR_APPEND);
			...
			char ***argv_for_execve)
{
	...
	while (token_list != NULL)
	{
		...
		{
			argv_for_execve[i] = (char **)malloc(sizeof(char *)
					* (word_quant + 1));
			word_quant = 0;
			i++;
		}
