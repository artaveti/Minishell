...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_list_creat_environment.c                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: artaveti <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/13 19:50:17 by artaveti          #+#    #+#             */
/*   Updated: 2024/01/13 19:59:33 by artaveti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				ft_additional_for_ft_list_new_for_environment(
						t_environment_list *result, char *string_from_envp);
...
t_environment_list	*ft_list_creat_start_of_environment(char *envp[])
{
	...
	else
	{
		...
		start_of_list->name_and_value[0] = ft_creat_first_part_of_word(
				envp[0], "=");
		start_of_list->name_and_value[1] = ft_strdup(ft_strchr(
					envp[0], '=') + 1);
	}
...
t_environment_list	*ft_list_new_for_environment(char *string_from_envp)
{
	...
	if (!ft_char_find('=', string_from_envp))
	{
		...
		ft_additional_for_ft_list_new_for_environment(result,
			string_from_envp);
			...
			t_environment_list *result, char *string_from_envp)
{
	...
	if (*(ft_strchr(string_from_envp, '=') - 1) == '+')
		result->name_and_value[0] = ft_creat_first_part_of_word(
				string_from_envp, "+");
	else
		result->name_and_value[0] = ft_creat_first_part_of_word(
				string_from_envp, "=");
