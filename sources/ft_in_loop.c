...
/*   Updated: 2024/01/13 21:44:08 by artaveti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lib_for_minishell.h"

void	ft_set_for_ft_loop_before_while(t_term_and_work_dir *term,
			int	*loop_stop_num, int *error_num);
void	ft_set_for_ft_loop_after_while(t_term_and_work_dir *term,
			char **input_str, int *loop_stop_num);
void	ft_check_errors_before_start_of_program(t_token_list **list,
			char *input_str, int *error_num);
void	ft_check_error_num_and_free_for_ft_loop(t_token_list *token_list,
			t_token_list *heredoc_list, char *input_str, int error_num);
			...
			t_environment_list *envp_list)
{
	...
	while (loop_stop_num == 0)
	{
		...
		if (loop_stop_num == 0)
		{
			...
			ft_check_errors_before_start_of_program(&token_list->next,
				input_str, &error_num);
			...
			ft_check_error_num_and_free_for_ft_loop(token_list, heredoc_list,
				input_str, error_num);
		}
	}
