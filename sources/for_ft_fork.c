...
void	ft_change_and_close_fd_in_fork(t_token_list *tmp_redir_list,
			t_for_prog *prog, t_for_fork *fk, int i)
{
	ft_change_stdin_stdout_fd_pipe(prog->fd_arr_pipe, prog->fd_quant_pipe, i);
	ft_change_stdin_stdout_fd_redir(tmp_redir_list, fk->fd_out,
		prog->fd_arr_heredoc, 0);
...
void	ft_execve(t_for_fork *fk, t_for_prog *prog, int i)
{
	...
	if (ft_char_find('/', prog->argv_for_execve[i][0]))
		execve(prog->argv_for_execve[i][0], prog->argv_for_execve[i],
			prog->envp_for_execve);
	fk->prog_paths = ft_prog_names_join(prog->path_arr,
			prog->argv_for_execve[i][0]);
	j = 0;
	while (fk->prog_paths != NULL && fk->prog_paths[j] != NULL)
	{
		if ((access(fk->prog_paths[j], F_OK) == 0))
			execve(fk->prog_paths[j], prog->argv_for_execve[i],
				prog->envp_for_execve);
		j++;
	}
	return ;
