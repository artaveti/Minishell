...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/13 18:50:36 by artaveti          #+#    #+#             */
/*   Updated: 2024/01/13 19:00:17 by artaveti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lib_for_minishell.h"

int		ft_for_cd_home_set(t_environment_list **envp, t_for_prog *prog,
			char **path, int fd_out);
int		ft_for_cd_check_is_path_wrong(t_for_prog *prog, char **path);
void	ft_for_cd_change_pwd_and_oldpwd_in_envp(t_environment_list **envp,
			t_for_prog *prog);
...
int	ft_for_cd_check_is_path_wrong(t_for_prog *prog, char **path)
{
	if (((prog->pwd_str[0] == NULL || !ft_strncmp(prog->pwd_str[0], ".", 2))
			&& !ft_strncmp(*path, "..", 3)) || chdir(*path) != 0)
	{
		if (prog->pwd_str[0] == NULL)
			printf(ERROR_CD_GETCWD_CANT_ACCESS);
